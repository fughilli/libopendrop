...
void main() {
  ...
  vec2 sampling_screen_uv = rotate(
      vec2(length(screen_uv), 0),
      mod_angle + sin(energy_scaled * 7.1) * sin(energy_scaled * 2.391) * 0.3);
  ...
  // a "darkening" effect.
  float brighten_darken_weight = (sin(energy_scaled) / 10 + 0.97);
